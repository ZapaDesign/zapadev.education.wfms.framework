...
class Category extends AppModel
{
    ...
    public function get_products($ids, $lang, $start, $perpage): array
    {
        ...
        $order_by = '';
        if (isset($_GET['sort']) && array_key_exists($_GET['sort'], $sort_value)) {
            $order_by = $sort_value[$_GET['sort']];
        }
        
        return R::getAll(
            "SELECT p.*, pd.* FROM product p JOIN product_description pd on p.id = pd.product_id WHERE p.status = 1 AND p.category_id IN ($ids) AND pd.language_id = ? $order_by LIMIT $start, $perpage",
            [$lang['id']]
        );
    }
